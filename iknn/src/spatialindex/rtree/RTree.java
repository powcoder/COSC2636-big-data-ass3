...
public class RTree implements ISpatialIndex
{
	...
	public RTree(PropertySet ps, IStorageManager sm)
	{
		...
		if (var != null)
		{
			...
			//System.out.println("Id: " + var.toString());
			try
			{
	...
	public void aggregatedNearestNeighborQuery(int k, double best_dist, final IShape query, final IVisitor v, final INearestNeighborComparator nnc)
	{
		if (query.getDimension() != m_dimension) throw new IllegalArgumentException("nearestNeighborQuery: Shape has the wrong number of dimensions.");
		...
		try
		{
			...
			while (! st.empty())
			{
				...
				else
				{
					...
					//System.out.println("I/O: "+ (end- start));
				}
			}
	...
	//------------------------------------------------------ update ----------------------------------------------------
	public void rangeIncrementalQuery(final IShape query, final IShape query_old, final IVisitor v)
	{
		m_rwLock.read_lock();
			...
			while (! st.empty())
			{
				...
				if (n.m_level == 0)
				{
					...
					for (int cChild = 0; cChild < n.m_children; cChild++)
					{
						...
						boolean c = false;
						if(query_old != null){
							c = query_old.contains(n.m_pMBR[cChild]);
						}						
	...
	public void rangeCellQuery(final IVisitor v)
	{
